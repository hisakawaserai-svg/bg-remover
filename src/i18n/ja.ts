...
const ja = {
  ...
  bgEngineChoice: {
    title: '背景除去の方式を選ぶ',
    subtitle: '迷ったら色ベースです。この画像にはどちらを使いますか？',
    floodDesc: '背景の色を基準に、同じような色をまとめて透過します。単色〜均一な背景の写真・イラスト向き。まずはこちらです。',
  ...
  settings: {
    ...
    bgEngine: '背景除去の方式',
    bgEngineHint: '色ベース（推奨）は背景色を基準に判定。被写体検出は背景が複雑な写真や人物の切り抜きに強い方式です。\n注意: 文字やロゴなど「被写体ではない」と判定した部分も消えることがあります。',
    bgEngineHintUnavailable: '被写体検出はこの端末のOSでは使えません。',
    bgEngineFlood: '色ベース（推奨）',
  ...
  howto: {
    ...
    sectionToolbar: '右の道具',
    sectionToolbarLead: '右上の今の道具を押すと、下にこの順で並びます。道具ごとに説明します。',
    sectionBar: '下のバーとヘッダー',
    ...
    editorEyedropSlider: '下のスライダーは数字の大小ではありません。左は似た色だけ、右は近い色まで広く消します。',
    editorRetrans: '右のきらきら（再透過）。消えすぎ／残りを、範囲を決めてやり直します。',
    editorRetransSlider: 'スライダーも数字ではありません。左は絵を残す（消しすぎない）、右は背景まで広く消します。',
    editorLoupeTitle: 'ルーペ',
    editorLoupeSize: '指で触っているところを拡大して見せます。ルーペをタップすると、大→中→小と大きさが変わります。',
    ...
    editorLoupe: '右の照準ボタン。設定の「編集操作モード」と同じです。',
    editorLoupeFixed: '指で直接狙ってなぞります。普段はこちらです。',
    editorLoupeAdjust: '画面に十字を出して、少しずつ狙います。細い線や角に使います。',
    ...
    resultNumbers: '見出し右の「1」。数字と注意マークが邪魔なら消します。透過の確認用です。',
