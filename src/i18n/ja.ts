...
const ja = {
  ...
  editor: {
    ...
    modeRestore: '復元ブラシ',
    modeRestoreHint: '消えすぎた部分をなぞって元に戻します。「消す」に切り替えると、なぞった部分を色に関係なく透過できます（スポイトで消えない孤立した部分に）',
    brushModeRestore: '戻す',
    brushModeErase: '消す',
    brushSize: 'ブラシの太さ',
    ghost: '元画像',
  ...
  settings: {
    ...
    bgEngine: '背景除去の方式',
    bgEngineHint: '色ベースは背景色を基準に判定。被写体検出(Vision)は背景が複雑な写真や人物の切り抜きに強い方式です。\n注意: Visionは文字やロゴなど「被写体ではない」と判定した部分も消えることがあります。',
    bgEngineHintUnavailable: '被写体検出(Vision)はiOS 17以降の実機でのみ使用できます。',
    bgEngineFlood: '色ベース',
    bgEngineVision: '被写体を検出(Vision)',
    confirmBgEngineEachTime: '画像を選ぶたびに確認する',
    confirmBgEngineEachTimeHint: 'OFFにすると、この方式に固定して毎回は聞きません。',
  ...
  howto: {
    ...
    step2Body:
      'セットアップ画面でモードを選びます。\n\n【自動分割】行数を確認・調整して「この行数で分割」。プレビューの分割線がずれていたら、指でドラッグして動かせます。\n\n【範囲を調整】タップで各キャラを直接囲んで切り出します。自動がうまくいかないときに使います。',
    step2Note: 'まず自動を試してみてください。自動で大半は揃います。',
    step3Title: 'STEP 3  結果を確認・調整する',
    step3Body:
      '分割結果を確認し、ズレや合体があれば調整します。\n\n• 合体している → 「戻る」で分割の細かさを上げて分割し直す\n• 隣のカットとまとめたい → カットを長押しして選択し「〇枚を合体する」\n• 1枚だけ直したい → カットをタップして範囲を調整\n• 選んだカットだけ共有したい → 長押しして選択し「〇枚を共有する」\n• 編集をやり直したい → 「リセット」で最初の分割結果に戻す',
    step3Note:
      '完璧でなくても「保存する」で透過PNGとして「{album}」アルバムに保存されます。オレンジの⚠が付いたカットは複数キャラが入っている可能性があり、保存時に確認が表示されます。\n段ごとに数が違う画像（例: 最後の段だけ列数が多い）は、自動分割が全段共通の線を使う都合で一部の段だけズレることがあります。その段のセルだけ合体または「範囲を調整」で直してください。',
    ...
    tip3: '自動でどうしても揃わない場合は「範囲を調整」でキャラを直接囲む（消し残しを直す細かいツールもここにあります）',
    noticeTitle: 'ご注意',
    notice1:
  ...
  errors: {
    ...
    visionUnsupported: 'この背景除去方式はiOS 17以降の実機でのみ使用できます。',
